...
export function isEmptyNumber(n: number): boolean {
    return n === null || n === undefined || Object.is(n, NaN) || typeof n !== 'number';
}

export function allEmpty(p: PlayerData): boolean {
    return isEmptyName(p.playerName) && isEmptyNumber(p.playerPlace) && isEmptyNumber(p.victoryPoints);
}
...
export const ERRORS = {
    NO_EMPTY_VALUES: 'Non-blank entries must have a player name, place, and victory points',
...
export function validateInput(input: GameData): string[] {
    ...
    if (numNonBlankEntries <= 1)
        errors.push(ERRORS.MINIMUM_ENTRIES);
    ...
    for (let x = 0; x < input['playerData'].length; x++) {
        ...
        if (!anyEmpty(player)) {
            ...
            if (player.playerPlace === lastPlace && player.victoryPoints !== lastScore) {
